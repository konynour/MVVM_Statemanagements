...
class FavoritesProvider with ChangeNotifier {
  final List<MoviesModels> _favoritesList = [];
  List<MoviesModels> get favoritesList => _favoritesList;
  final favskey = "favsKey";
  bool isFavorite(MoviesModels movieModel) {
    return _favoritesList.any((movie) => movie.id == movieModel.id);
  }

  Future<void> addOrRemoveFromFavorites(MoviesModels movieModel) async {
  ...
  Future<void> loadFavorites() async {
    ...
    _favoritesList.addAll(
      stringList.map((movie) => MoviesModels.fromJson(json.decode(movie))),
    );
    notifyListeners();
  }
